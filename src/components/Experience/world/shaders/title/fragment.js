const fragment = /* glsl */ `
  ...
  void main() {
    ...
    vec3 blue = vec3(0.137, 0.169, 0.306);  // rgb(42, 58, 124) in 0-1 range
    vec3 purple = vec3(0.5, 0.0, 0.5);     // rgb(241, 10, 241) in 0-1 range
    ...
    vec4 l3 = vec4(purple, outset * uOpacity);  // Changed from darkBlue to purple
    vec4 l4 = vec4(white, border * 0.6 * uOpacity);
    vec4 l5 = vec4(blue, border * uOpacity);    // Using the darker blue from gradient
    vec4 l6 = vec4(blue, outset * uOpacity);    // Using the darker blue from gradient
   float mixFactor = vLayoutUv.x; // or vLayoutUv.y depending on gradient direction
vec3 gradientColor = mix(purple, blue, mixFactor);
vec4 l7 = vec4(gradientColor, outset * uOpacity);
